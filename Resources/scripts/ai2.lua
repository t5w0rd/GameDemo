...
function LuaAIWarrior:onUnitTick(unit, dt)
    ...
    if t and t:getDistance(unit) < 200 then
        self:tryCastSkill(unit, "·ç±©Õ½´¸", t)
    end
	if t and t:getHp() > 60 then
		self:tryCastSkill(unit, "¿ñ±©", nil, 100)
		self:tryCastSkill(unit, "·ßÅ­ÖØÌ¤", nil, 100)
		self:tryCastSkill(unit, "Á¬»·±¬ºä", nil, 100)
		self:tryCastSkill(unit, "Ó¢ÓÂ¼²Ô¾", nil, 100)
	end
